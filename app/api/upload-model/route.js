...
async function POST(request) {
    ...
    if (!session) {
        ...
        return new NextResponse({ status: 401, body: { message: 'You are not authenticated!' } });
    }
    ...
    try {
        ...
        if (!file)
            return new NextResponse({ status: 401, body: { message: 'No file detected' } });

        const isFile = file instanceof File;

        if (!isFile)
            return new NextResponse({ status: 401, body: { message: 'File type not of File Type' } });
        ...
        console.log(error);
        return new NextResponse({ status: 500, body: { error: error, message: 'Internal server error' } });
    }
}
